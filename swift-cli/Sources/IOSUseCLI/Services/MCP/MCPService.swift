...
enum MCPService {
    ...
    Use js for persistent JavaScript iOS automation. Start with await cua.getState() to discover Devices, or let device = await cua.getDevice(knownDeviceID). Selection emits the initial AX state and API guidance. Keep handles and helper functions across calls. Batch known actions and fresh observations inside one call; getAXState({emit:false}) keeps intermediate state out of the response. Use nodeRepl.write for results and nodeRepl.emitImage for images; observation APIs emit automatically by default. Do not use shell polling or ios-use repl. Device setup and Driver start/stop remain native ios-use CLI commands. js_reset clears JavaScript state without stopping Drivers or Apps. One server process owns one JavaScript context; use separate processes for independent Agent conversations.
    """
