...
enum MCPService {
    ...
    Use js for persistent JavaScript iOS automation. Start with await cua.getState() to discover Devices, or let device = await cua.getDevice(knownDeviceID). Selection emits initial AX and API guidance; use {observe:false} for an idle Device, then device.start(). Keep handles and helpers across calls. Batch known flows in one call: getAXSnapshot() returns full structured state without text/diff/output; waitFor(ax => condition, {timeout:10}) polls inside JavaScript and throws on timeout. Check the expected page, not whole-text equality. Emit compact results with nodeRepl.write; getAXState and screenshot APIs emit by default. Use device.listApps(), getApp(bundleId), terminateApp(bundleId), start()/stop() for lifecycle; signing/setup remains native CLI. Do not use shell polling or ios-use repl. js_reset clears JavaScript without stopping Drivers or Apps. One server process owns one context; use separate processes for independent Agent conversations.
    """
