...
final class TCPAttachTests: XCTestCase {
    ...
    func testAttachRoutesTwoEndpointsAndDetachLeavesDriverAlive() throws {
        try withPaths { paths in
            ...
            for (command, id, server) in [("start", "remote-a", first), ("attach", "remote-b", second)] {
                let result = cli.run(arguments: [command, "--device", id,
            ...
            XCTAssertEqual(cli.run(arguments: ["stop", "--device", "remote-a"]).exitCode, 0)
    ...
    func testRejectedProtocolDoesNotPersistAttachment() throws {
        try withPaths { paths in
            ...
            defer { server.stop() }
            let cli = IOSUseCLI(pathsForTesting: paths)
            let result = cli.run(arguments: ["start", "--device", "rejected", "--host", "127.0.0.1",
    ...
    func testOfflineAttachmentDoesNotStartLocalRuntimeAndStopOnlyDetaches() throws {
        try withPaths { paths in
            ...
            let cli = IOSUseCLI(pathsForTesting: paths)
            XCTAssertEqual(cli.run(arguments: ["start", "-d", "offline", "--host", "127.0.0.1", "--port", String(server.port)]).exitCode, 0)
            server.stop()
            var launches = 0
            var terminations = 0
            ...
            XCTAssertEqual(cli.run(arguments: ["stop", "-d", "offline"]).exitCode, 0)
    ...
    func testLocalSessionCannotBeDetachedAndInvalidInputsCreateNoSession() throws {
        try withPaths { paths in
            ...
            for command in ["start", "attach"] {
                for arguments in [
                    [command, "--host", "localhost", "--port", "8102"],
                    [command, "--device", "mac", "--host", "localhost", "--port", "8102"],
                    [command, "--device", "remote", "--host", "localhost", "--port", "65536"],
                    [command, "--device", "remote", "--host", "http://localhost", "--port", "8102"],
                ] {
