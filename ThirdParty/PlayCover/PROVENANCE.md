...
| --- | --- |
| `Entitlements.composeEntitlements(self)` | Both paths call the vendored `Entitlements.composeEntitlements` directly on the staged `BaseApp`. Explicit `discordActivityEnabled`, `bypass`, `playSignActive`, and managed-home inputs replace GUI `AppSettings` reads. The production path then preserves every original key and appends only the managed Runtime socket/PlayChain sandbox rules; this strengthens capability retention without replacing the pinned composer baseline. |
...
| `Shell.signAppWith(executable, entitlements: tmpEnts)` | The pinned oracle calls `Shell.signAppWithPinnedOracle` with the exact upstream root target and `--deep` arguments. Production calls the retained `Shell.signAppWith` primitive explicitly for entitlement-bearing code objects, signs all other child Mach-Os with `Shell.signMacho`, verifies each child, signs the root `.` last with the final composed entitlements, then re-verifies the whole order. Removing `--deep` from the production primitive is what makes the upstream root sign deterministic and preserves nested extension entitlements. |
