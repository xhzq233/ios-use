...
public final class Macho {
    public static func stripBinary(_ binary: inout Data) throws {
        ...
        let isFat32 = header.magic == FAT_MAGIC
            || header.magic == FAT_CIGAM
        let isFat64 = header.magic == FAT_MAGIC_64
            || header.magic == FAT_CIGAM_64
        ...
        if isFat32 || isFat64 {
            ...
            for _ in 0..<header.nfat_arch {
                if isFat64 {
                    guard offset + MemoryLayout<fat_arch_64>.size
                            <= binary.count else {
                        throw PlayCoverError.failedToStripBinary
                    }
                    var arch = binary.extract(
                        fat_arch_64.self,
                        offset: offset
                    )
                    if shouldSwap {
                    ...
                    if arch.cputype == CPU_TYPE_ARM64 {
                        print("Found ARM64 arch in fat64 binary")
                        guard arch.offset <= UInt64(binary.count),
                              arch.size <= UInt64(binary.count) - arch.offset,
                              arch.offset <= UInt64(Int.max),
                              arch.size <= UInt64(Int.max) else {
                            throw PlayCoverError.failedToStripBinary
                        }
                        let start = Int(arch.offset)
                        binary = binary.subdata(
                            in: start..<(start + Int(arch.size))
                        )
                        return
                    }
                    offset += MemoryLayout<fat_arch_64>.size
                } else {
                    guard offset + MemoryLayout<fat_arch>.size
                            <= binary.count else {
                        throw PlayCoverError.failedToStripBinary
                    }
                    var arch = binary.extract(
                    ...
                    if arch.cputype == CPU_TYPE_ARM64 {
                        ...
                        let start = UInt64(arch.offset)
                        let size = UInt64(arch.size)
                        guard start <= UInt64(binary.count),
                              size <= UInt64(binary.count) - start else {
                            throw PlayCoverError.failedToStripBinary
                        }
                        binary = binary.subdata(
                            in: Int(start)..<Int(start + size)
                        )
                        return
                    }
    ...
    public static func convertMacho(_ macho: URL) throws {
        print("Converting MachO at \(macho.path)")

        var binary = try Data(contentsOf: macho)
        let originalAttributes = try FileManager.default.attributesOfItem(atPath: macho.path)
        ...
        if let permissions = originalAttributes[.posixPermissions] {
            try FileManager.default.setAttributes(
                ...
                ofItemAtPath: macho.path
            )
        }
    }

    public static func replaceLibraries(_ binary: inout Data) throws {
    ...
    public static func replaceLibrary(_ binary: inout Data, _ rpath: String, _ lib: String) throws {
    ...
    public static func replaceVersionCommand(_ binary: inout Data) throws {
    ...
    public static func replaceLastCommand(_ binary: inout Data,
        ...
        let headerShouldSwap = header.magic == MH_CIGAM_64
        if headerShouldSwap {
        ...
        if headerShouldSwap {
            swap_mach_header_64(&outputHeader, NX_BigEndian)
        }
        let newHeaderData = Data(bytes: &outputHeader, count: headerSize)
        binary.replaceSubrange(0..<headerSize, with: newHeaderData)
    }

    public static func iterateLoadCommands(binary: Data, _ evaluate: (Int, Bool) -> Bool) throws -> Int {
        let headerSize = MemoryLayout<mach_header_64>.size
        var header = binary.extract(mach_header_64.self)
    ...
    public static func isMachoEncrypted(atURL url: URL) throws -> Bool {
    ...
    public static func isMachoValidArch(_ url: URL) throws -> Bool {
        var binary = try Data(contentsOf: url)
        try stripBinary(&binary)
        var result = false
        _ = try iterateLoadCommands(binary: binary) { offset, shouldSwap in
