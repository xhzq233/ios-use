...
/// In upstream PlayTools this implementation lives in AKInterface.bundle so it
/// can link AppKit. ios-use keeps the pinned Plugin boundary but routes the
/// same observations/policy through a dynamic AppKit bridge compiled into the
/// single injected Runtime framework.
final class AKPlugin: NSObject, Plugin {
    ...
    var screenCount: Int {
        IOSUsePlayAppKitBridge.screenCount()
    }

    var mousePoint: CGPoint {
        IOSUsePlayAppKitBridge.mousePoint()
    }

    var windowFrame: CGRect {
        IOSUsePlayAppKitBridge.windowFrame()
    }

    var mainScreenFrame: CGRect {
        IOSUsePlayAppKitBridge.mainScreenFrame()
    }

    var isMainScreenEqualToFirst: Bool {
        IOSUsePlayAppKitBridge.isMainScreenEqualToFirst()
    }

    var isFullscreen: Bool {
        IOSUsePlayAppKitBridge.isFullscreen()
    }
    ...
    func setupKeyboard(
        keyboard _: @escaping (UInt16, Bool, Bool, Bool) -> Bool,
        swapMode _: @escaping () -> Bool
    ) {}

    func setupMouseMoved(
        _ mouseMoved: @escaping (CGFloat, CGFloat) -> Bool
    ) {
        _ = mouseMoved
    }

    func setupMouseButton(
        left _: Bool,
        right _: Bool,
        _ consumed: @escaping (Int, Bool) -> Bool
    ) {
        _ = consumed
    }

    func setupScrollWheel(
        _ onMoved: @escaping (CGFloat, CGFloat) -> Bool
    ) {
        _ = onMoved
    }

    func urlForApplicationWithBundleIdentifier(
        _ value: String
    ) -> URL? {
        _ = value
        return nil
    ...
    func setMenuBarVisible(_ value: Bool) {
        IOSUsePlayAppKitBridge.setMenuBarVisible(value)
    }
}
