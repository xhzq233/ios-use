...
Latest measurement: **2026-09-20**, PR #27 at `aca8dc0b`. USB real-device and Mac Backend timings are reported separately. Lower latency is better.

The latest run covers **swipe and drag only**. Appium/WDA, DOM, screenshots, taps, input, app lifecycle and session startup were not rerun in this update. Their older measurements remain dated historical records below; do not combine old WDA timings with new ios-use timings to calculate a current speedup.

## Latest swipe measurements (2026-09-20)

Before: main `815132ff`. After: `aca8dc0b`. Both are Release builds with matching CLI and XCTest Driver or Mac Runtime. The before group is the immediate pre-PR main, **not the published v2.1.0 binary**. The comparison isolates PR #27; earlier DOM and idle-wait improvements are present in both groups.
...
macOS 15.7.7 arm64, the same Release UIKit Fixture and iPhone 13 layout (390×844) for both groups. App-written offsets, selected rows and delivered touch endpoints verify effects independently of command success. All trials reset the three scroll containers first.
...
### Comparison with the May 30 record

| USB Settings case | May 30 mean (ms) | September 20 mean (ms) | Recorded latency reduction |
| --- | ---: | ---: | ---: |
| Default scroll, 200 pt | 2170.6 | 2013.3 | 7.2% |
| Find offscreen Developer row | 10799.2 | 9471.5 | 12.3% |

The May run used three samples per command; the September group used six. The May binary was not rerun alongside the current version, and device, OS and page-state differences were not controlled across dates. This is a comparison of historical records, not an isolated measurement of code improvement. The controlled before/after comparison is the September run above.

## Historical screenshot backend probe (2026-07-14)
...
## Historical Appium/WDA comparison (2026-05-30)

This historical run compared `ios-use` against the full `Appium Server -> WebDriverAgent` stack on the same real-device Settings scenario. Lower latency is better.
...
![Historical ios-use and Appium + WDA latency comparison, measured May 30, 2026](benchmark.svg)
...
- `dom_cached` is a historical case name. Current `dom` defaults to diff/unchanged output; any new comparison against WDA full source must explicitly use `dom --nodiff`. The May number does not measure the current DOM implementation.
...
The former `find_*` benchmark cases were removed with the public `find` command; use `waitFor` for read-side existence checks and action commands for semantic targeting. Numbers vary by device, iOS version, app state, and whether the target page is already warm. These historical latency reductions are not a measurement of the current version.
