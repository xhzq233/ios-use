...
| `scripts/test_playcover_backend.sh --non-live` | Unified Apple-silicon hosted gate: upstream audit, fresh workspace CLI/Runtime build and analysis, fixture build, non-GUI Simulator-scale host contract, compositor/PlayChain smoke, vendored Swift tests, and release-installed execution. `--live` adds the public fixture matrix, isolated Runtime protocol/crash stress, and configured generic external-App live gate; it requires the private runner inputs. |
| `scripts/test_playcover_fixture_live.sh [--static\|--live]` | `--static` verifies the ordinary opaque NSWindow/fixed-canvas contract without launching an App or requiring a GUI. `--live` runs the fixture matrix in an unlocked GUI session, including a system title bar, two proportional host resizes, one display scale, canvas-only capture, inverse-scale global mouse, and title-bar miss-hit checks. |
...
| `playcover-fixtures/test_simulator_scale_host_contract.sh --static` | Hermetic source contract for an ordinary opaque system NSWindow, zero synthetic spacer, 430:932 proportional resize, fixed UIKit geometry, canonical Runtime geometry, canvas-only crop, and live-gate mode/mapping policy. |
