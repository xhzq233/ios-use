---
name: "ios-use-skill"
description: "Use ios-use to drive iOS devices via CLI. Covers auto-session, device management, UI element inspection (dom/find), tap/swipe/input actions, oslog/nslog, app lifecycle, HTTP/HTTPS proxy capture, and YAML flow authoring. Use this skill when the user wants to interact with an iOS device, inspect screen elements, automate UI steps, capture network traffic, write or debug automation flows, or check device logs."
---
...
- 如果 `ios-use devices` 显示 `driver update required`，说明本机 CLI 已升级但设备上的 driver 还是旧版本，必须重新执行 `ios-use config --udid <udid>`。
...
- 任意操作命令（dom/tap/swipe/...）会自动通过本机 daemon 建连并复用 driver，无需手动 `session start`
...
无需手动建连——首次执行任意操作命令（dom、tap 等）时自动建连。需要操作特定 app 时先 `activateApp`：
...
```bash
ios-use stop                        # 停止当前 driver 和 IOS_USE_HOME 对应的 daemon
```
