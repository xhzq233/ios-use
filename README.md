...
ios-use activateApp com.apple.Preferences --device <device-id>
ios-use dom --device <device-id>
```

Each `start` creates an independent Device Context inside the same `IOS_USE_HOME`. When exactly one Device is running, screen-driving commands may omit `--device`; with multiple running Devices, use the stable Device IDs printed by `status`.
