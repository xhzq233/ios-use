...
int main(int argc, char **argv) {
    @autoreleasepool {
        ...
        BOOL retarget = [mode isEqual:@"retarget-metallib"];
        BOOL computeOnly = [mode hasPrefix:@"compute-"] || retarget;
        ...
        if (![@[@"clear", @"surface", @"source", @"metallib", @"compute-source", @"compute-metallib", @"retarget-metallib"] containsObject:mode]
            || (useMetallib && argc != (retarget ? 4 : 3))) return 2;
        ...
        NSError *error = nil;
        if (retarget) {
            id<MTLLibrary> original = [device newLibraryWithURL:[NSURL fileURLWithPath:@(argv[2])] error:&error];
            id<MTLFunction> originalFunction = [original newFunctionWithName:@"probe"];
            id<MTLComputePipelineState> originalPipeline = originalFunction
                ? [device newComputePipelineStateWithFunction:originalFunction error:&error] : nil;
            if (!original || !originalFunction || originalPipeline || !error) return 55;
            NSLog(@"[probe] device library pipeline rejected: %@", error);
            error = nil;
        }
        id<MTLLibrary> library;
        if (useMetallib) {
            library = [device newLibraryWithURL:[NSURL fileURLWithPath:@(argv[retarget ? 3 : 2])] error:&error];
        } else {
