...
static void showLegacyWindow(UIWindow *window, SEL selector) {
    if (!window.windowScene) {
        ...
        // This late scene bootstrap lacks the normal launch initialization
        // context. Adopt windows created earlier by a legacy AppDelegate.
        if (scenes.count == 1) {
            window.windowScene = scenes.firstObject;
            NSLog(@"[scene-bootstrap] associated legacy window with local scene");
        }
    }
